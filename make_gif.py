...
for k in tqdm(range(num_ite)):
    ...
    ax.scatter(x_source, y_source, c='r', s=100)
    ax.set_ylim([1000, 9000])
    plt.savefig(f"./gifs_2D/{k}", dpi=200)
    plt.close()

listdir = [str(a) + ".png" for a in sorted(int(c[:-4]) for c in os.listdir("./gifs_2D/"))]
...
#         stations_z,
#         c='g', s=5)
#     ax.scatter3D(x_source, y_source, z_source, c='r', s=100)
