...
current_max = 0
for key1 in sum_sol.keys():
    for key2 in sum_sol.keys() :
        dist = (((station_pos[int(key1)]["x"]- station_pos[int(key2)]["x"])**2) + ((station_pos[int(key1)]["y"]- station_pos[int(key2)]["y"])**2))**(1/2)
        corrected_value = g(dist) * sum_sol[key2]
        if sum_sol[key1] + corrected_value > current_max and key1 != key2:
            current_max = sum_sol[key1] + corrected_value
            best_bal1 = int(key1)
            best_bal2 = int(key2)
            knownDist = dist
...
ax.scatter3D(station_pos[best_bal1]["x"],station_pos[best_bal1]["y"],station_pos[best_bal1]["z"], color = "k")
ax.scatter3D(station_pos[best_bal2]["x"],station_pos[best_bal2]["y"],station_pos[best_bal2]["z"], color = "k")
plt.show()