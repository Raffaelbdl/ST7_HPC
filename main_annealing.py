...
min, max, step = -100., 100.01, 1
clips = np.array([[min, max], [min, max]])

# with 2 artificial minimas
# f, f_min = generate_random_interpolation(min, max, step)
f, f_min = generate_sinusoidal_interpolation(intensity=1, period=50, offset=25, min=min, max=max, step=step)
...
fig, ax = plot_interpolation(f, min, max, step, 0.5)
